...
?>
		...
		<?if ( !$noShowForm ):?>
			...
			<form id="register_form" name="REGISTER" method="POST" class="form form_register popup__form" action="<?= POST_FORM_ACTION_URI ?>" >
				...
				<input type="hidden" class="api-mf-antibot" value="" name="ANTIBOT[NAME]">
					...
					<?/* 						<div class="g-recaptcha" data-sitekey="6Lc51bIUAAAAALfSSkyOmsn8sOtMrzbs5zCUAx1f"></div>		if ($arResult["USER_PROPERTIES"]["SHOW"] == "Y"): ?>
						<div class="field">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#politic').html('<label><input type="checkbox" name="agree_politic"  <?if ( $arResult['arrVALUES']['agree_politic'] == "on" ):?>checked<?endif;?>>Я согласен с <a href="/politika-konfidentsialnosti/" target="_blank">политикой конфиденциальности</a></label>');
