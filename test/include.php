    ...
    public static function SendPaidOrdersToEvotor(){
        if(PKASSAMainSettings::GetSiteSetting("AUTOMAT")=="Y" && CModule::IncludeModule("sale")){
            ...
            while($arOrders = $rsOrders->Fetch()){
                $skip = false;
                $order = Sale\Order::load($arOrders["ID"]);
                ...
                if(count($paymentIds)>1){
                    ...
                    foreach ($paymentCollection as $payment){
                        if($payment->isPaid()){
                            ...
                            if(!in_array($psID,$arSelectedPaySystems))$skip = true;
                        }
                    }
                }
                if($skip){
                    $ExportedOrders[] = $arOrders["ID"];
        ...
        if(CModule::IncludeModule("sale")){
            ...
            if($arOrder){
                ...
                if($arOrder["PRICE_DELIVERY"]){
                    $dVat = PKASSAMainSettings::GetSiteSetting("DELIVERY_NDS")?PKASSAMainSettings::GetSiteSetting("DELIVERY_NDS"):0;
                    $dVatSum = round(($arOrder["PRICE_DELIVERY"]/(1 + $dVat/100))*$dVat/100, 2);
                    $arBasketItems[] = array(
                ...
                if( PKASSAMainSettings::GetSiteSetting("DOMAIN") ){
                    $server_name = PKASSAMainSettings::GetSiteSetting("DOMAIN");
                }else{
                    $server_name = $_SERVER["SERVER_NAME"];
                    COption::SetOptionString(static::$module, "DOMAIN", $server_name);
                }
                $site_name = preg_replace("/[^\p{Latin}]/ui",'',$server_name);
                if(!$site_name) $site_name = substr(md5($server_name),0,10);
                $payType = 1;
...
class PKASSAMainSettings
{
    ...
    public static function GetSiteSetting($key) {
        if($key=="EXPORTEDORDERS" && file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/pavelbabich.kassa/exportedorders.php")){
            ...
            return $value;
        }elseif($key=="PAIDORDERS" && file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/pavelbabich.kassa/expaidorders.php")){
            $value = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/pavelbabich.kassa/expaidorders.php");
            return $value;
        }else{
