...
class RetailCrmOrder
{
    ...
    {      
		...
		if ( $arFields['STATUS_ID'] == "NA" ){
			$orderType = 'magazin-a-15';
		}
		elseif ( $arFields['STATUS_ID'] == "NM" ){
			$orderType = 'magazin-v-12';
			$store = 'B12/4';
		}
		elseif ( $arFields['STATUS_ID'] == "NB" ){
			$orderType = 'magazin-tt';
			$store = 'TT4/5';
		}
		elseif ( $arFields['STATUS_ID'] == "NC" ){
			$orderType = 'magazin-d-29';
			$store = 'D29';
		}
		elseif ( $arFields['STATUS_ID'] == "ND" ){
			$orderType = 'magazin-a-15';
		}
		elseif ( $arFields['STATUS_ID'] == "N" ){
			$orderType = 'magazin-a-15';
		}
		elseif(isset($arParams['optionsOrderTypes'][$arFields['PERSON_TYPE_ID']])) {
			$orderType = "";
		}
		...
		//открываем файл с массивом отправленных ранее заказов
		$listFile = $_SERVER["DOCUMENT_ROOT"]."/tools/files/retail.txt";
		$data = file_get_contents($listFile);
		$arOrders = unserialize( $data );

		$arCountOrders = array_count_values($arOrders);


		if ( !in_array($arFields['ID'], $arOrders) || $arCountOrders[$arFields['ID']] > 1 ){
				unset($order['status']);
		}
			...
			if($send) {
				...
				$fd = fopen( $_SERVER["DOCUMENT_ROOT"]."/tools/files/retail.txt", 'w');
				fwrite($fd, serialize($arOrders) );
				fclose($fd);
			}
